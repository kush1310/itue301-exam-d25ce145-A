 ...
 *
 * Populates MongoDB Atlas with production-ready real relational data:
 * - Customers:
 *   1. Kush Shah (kush@charusat.edu.in)
 *   2. Shah Kush (shah@charusat.edu.in)
 * - 6 Campus Restaurants with Authentic Menus
 * - 6 Dedicated Restaurant Owner Accounts (linked to each canteen)
 * - 1 System Administrator
 * - Dedicated Real Order History for each customer and canteen
 */
...
const runSeed = async () => {
  ...
  const customer2 = await Customer.create({
    name: 'Shah Kush',
    email: 'shah@charusat.edu.in',
    password: 'password123',
    phone: '+91 98765 43211',
    address: 'Hostel Block B, Room 108, CHARUSAT Campus, Changa',
    role: 'Customer'
  });
  ...
  // 4. Create Dedicated Real Order History for Kush (kush@charusat.edu.in)
  await Order.create({
    customerId: customer1._id,
    restaurantId: createdRestaurants[0]._id, // The Rustic Oven Bistro
    items: [
  ...
  await Order.create({
    customerId: customer1._id,
    restaurantId: createdRestaurants[1]._id, // Spice Symphony Tandoor
    items: [
  ...
  // 5. Create Dedicated Real Order History for Shah (shah@charusat.edu.in)
  await Order.create({
    customerId: customer2._id,
    restaurantId: createdRestaurants[4]._id, // Campus Brew & Bakery
    items: [
      { name: 'Hazelnut Iced Latte', quantity: 2, price: 160 },
      { name: 'Grilled Paneer Focaccia Sandwich', quantity: 1, price: 190 }
    ],
    totalAmount: 510,
    deliveryAddress: 'Hostel Block B, Room 108, CHARUSAT Campus, Changa',
    status: 'delivered'
  });

  await Order.create({
    customerId: customer2._id,
    restaurantId: createdRestaurants[2]._id, // Zen Dragon Express
    items: [
      { name: 'Steamed Veg Dimsums (6 pcs)', quantity: 2, price: 190 },
      { name: 'Hakka Noodles Deluxe', quantity: 1, price: 210 }
    ],
    totalAmount: 590,
    deliveryAddress: 'Hostel Block B, Room 108, CHARUSAT Campus, Changa',
    status: 'pending'
  ...
  console.log('[SEED] Dedicated real orders created for both customers and canteens.');
...
const executeWithRetry = async (attempts = 3) => {
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      console.error(`[SEED_ATTEMPT_${i + 1}_FAILED] ${err.message}`);
      if (i < attempts - 1) {
        console.log('[SEED_RETRY] Retrying database seeding in 2000ms...');
